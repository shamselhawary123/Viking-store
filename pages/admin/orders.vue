<template>
  <section class="space-y-6">
    ...
    <div class="overflow-hidden rounded-3xl border border-white/10 bg-[#111111]">
        ...
        <table class="w-full min-w-[980px] text-left text-sm">
          ...
          <tbody class="divide-y divide-white/10">
            <tr v-for="order in orders" :key="order.id">
              <td class="px-5 py-4 font-bold">#{{ order.id.slice(0, 8) }}</td>
              <td class="px-5 py-4">{{ order.full_name || "Customer" }}</td>
              <td class="px-5 py-4 text-gray-400">{{ order.phone || "-" }}</td>
              <td class="px-5 py-4 text-gray-400">{{ order.city || "-" }}</td>
              <td class="px-5 py-4">
                <select
              ...
              <td class="px-5 py-4 text-right font-black">${{ order.total_price }}</td>
      ...
      <p v-if="!loading && !orders.length" class="p-6 text-sm text-gray-500">
        No orders found.
      </p>
    </div>
      ...
      <div class="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-3xl border border-white/10 bg-[#111111] p-6">
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <h3 class="mt-2 text-2xl font-black">#{{ selectedOrder.id.slice(0, 8) }}</h3>
          </div>
          <button class="text-gray-400 hover:text-white" @click="selectedOrder = null">
            Close
          </button>
        </div>
        ...
        <div class="mt-6 grid gap-4 md:grid-cols-2">
          <div class="rounded-2xl bg-black p-4">
            ...
            <p class="mt-2 font-bold">{{ selectedOrder.full_name || "Customer" }}</p>
          </div>
          <div class="rounded-2xl bg-black p-4">
            <p class="text-sm text-gray-500">Phone</p>
            <p class="mt-2 font-bold">{{ selectedOrder.phone || "-" }}</p>
          </div>
          <div class="rounded-2xl bg-black p-4">
            <p class="text-sm text-gray-500">City</p>
            <p class="mt-2 font-bold">{{ selectedOrder.city || "-" }}</p>
          </div>
          <div class="rounded-2xl bg-black p-4">
            <p class="text-sm text-gray-500">Address</p>
            <p class="mt-2 font-bold">{{ selectedOrder.address || "-" }}</p>
          </div>
          <div class="rounded-2xl bg-black p-4">
            <p class="text-sm text-gray-500">Status</p>
            <p class="mt-2 font-bold capitalize text-[#FF4D00]">
              {{ selectedOrder.status || "pending" }}
            </p>
          </div>
          <div class="rounded-2xl bg-black p-4">
            <p class="text-sm text-gray-500">Total</p>
            <p class="mt-2 text-2xl font-black">${{ selectedOrder.total_price }}</p>
          </div>
        </div>

        <div class="mt-6 rounded-2xl bg-black p-4">
          <p class="text-sm text-gray-500">Notes</p>
          <p class="mt-2 text-gray-300">{{ selectedOrder.notes || "-" }}</p>
        </div>
        ...
        <div class="mt-6">
          ...
          <div class="mt-4 space-y-3">
            <div
              ...
              :key="item.id"
              class="flex flex-col gap-4 rounded-2xl bg-black p-4 md:flex-row md:items-center"
            >
              <img
              ...
              <p class="font-black text-[#FF4D00]">${{ item.product_price }}</p>
            </div>
            <p v-if="!itemsLoading && !orderItems.length" class="text-sm text-gray-500">
