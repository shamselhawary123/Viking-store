<template>
  <section class="container-premium section-premium">
    ...
    >
      ...
      >
        ...
        <button
          ...
          class="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-red-500/30 px-5 font-extrabold text-red-300 transition duration-200 hover:-translate-y-0.5 hover:bg-red-500 hover:text-white active:scale-[0.98]"
    ...
    <div
      v-if="!cartStore.items.length"
      class="premium-panel relative flex min-h-[520px] flex-col items-center justify-center overflow-hidden rounded-2xl p-8 text-center"
    >
      <img
      ...
      />
      <div
        class="relative flex h-20 w-20 items-center justify-center rounded-full border border-white/10 bg-black text-[#CF1D1D]"
      >
        <Icon name="i-heroicons-shopping-bag" class="text-4xl" />
      </div>
      <div class="relative">
        <h2 class="text-4xl font-black text-white">{{ t("cart.empty") }}</h2>
        <p class="mx-auto mt-3 max-w-md leading-7 text-neutral-400">
          {{ t("cart.emptyText") }}
        </p>
      </div>
      <NuxtLink
        to="/shop"
        class="premium-button premium-button-primary relative"
          ...
          >
            <div class="flex flex-col gap-5 md:flex-row">
              <img
                ...
                width="320"
                height="320"
                class="h-48 w-full rounded-xl object-cover md:h-40 md:w-40"
                loading="lazy"
                decoding="async"
              />
              ...
              <div class="flex flex-1 flex-col justify-between gap-5">
                <div>
                  <p class="eyebrow text-[0.65rem]">
                    {{ item.category || t("shop.combatGear") }}
                  </p>
                  <h2 class="mt-2 text-2xl font-black">{{ item.title }}</h2>
                  <div
                    class="mt-4 flex flex-wrap items-center gap-4 text-sm text-neutral-400"
                  >
                    <span class="inline-flex items-center gap-2">
                      <span
                        v-if="item.colorValue"
                    ...
                    <span
                      class="rounded-full border border-white/10 px-3 py-1 font-bold text-white"
                      >{{ item.size }}</span
                    >
                  </div>
                </div>
                ...
                >
                  <div
                    class="flex w-fit items-center overflow-hidden rounded-full border border-white/10 bg-white/[0.03]"
                  >
                    <button
                      class="flex h-12 w-12 items-center justify-center transition hover:bg-white/10 active:scale-95"
                    ...
                    <div
                      class="flex h-12 min-w-16 items-center justify-center border-x border-white/10 font-black"
                    >
                      {{ item.quantity }}
                    </div>
                    <button
                  ...
                  >
                    <div class="text-right">
                      ...
                      <h3 class="text-2xl font-black text-[#CF1D1D]">
                        {{
                    ...
                    <button
                      class="flex h-12 w-12 items-center justify-center rounded-full border border-red-500/30 text-red-300 transition hover:-translate-y-0.5 hover:bg-red-500 hover:text-white active:scale-95"
      ...
      <div class="lg:col-span-4">
        <aside class="premium-panel sticky top-28 rounded-2xl p-6">
          ...
          <p
            ...
            class="mt-5 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm font-bold text-red-200"
          >
            ...
            <div class="rounded-2xl border border-white/10 bg-black/25 p-4">
              <div class="flex items-start gap-3">
                <Icon
                  ...
                  class="mt-1 text-xl text-[#CF1D1D]"
                />
                <div>
          ...
          <div class="mt-5 grid grid-cols-3 gap-2 text-center">
            ...
            >
              ...
              <p
                class="mt-2 text-[0.65rem] font-bold uppercase tracking-[0.12em] text-neutral-400"
              >
                {{ t(badge.labelKey) }}
              </p>
            </div>
