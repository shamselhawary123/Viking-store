<template>
  <section class="container-premium section-premium">
    <div class="mb-10 rounded-2xl border border-white/10 bg-white/[0.03] p-6 md:p-8">
      ...
      <h1 class="display-heading mt-3 text-6xl text-white md:text-8xl">{{ t('checkout.title') }}</h1>
      <p class="mt-4 max-w-2xl leading-7 text-neutral-400">{{ t('checkout.lead') }}</p>
    </div>

    <div v-if="!cartStore.items.length" class="premium-panel relative overflow-hidden rounded-2xl p-10 text-center md:p-16">
      <img src="https://images.unsplash.com/photo-1517438476312-10d79c077509?q=60&w=900&auto=format&fit=crop" alt="" width="900" height="600" class="absolute inset-0 h-full w-full object-cover opacity-15" loading="lazy" decoding="async" />
      ...
      <h2 class="relative mt-6 text-3xl font-black text-white">{{ t('cart.empty') }}</h2>
      <p class="relative mx-auto mt-3 max-w-md text-neutral-400">{{ t('cart.emptyText') }}</p>
      <NuxtLink to="/shop" class="premium-button premium-button-primary relative mt-8">{{ t('common.products') }}</NuxtLink>
    </div>
    ...
    <div v-else class="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
      <form class="space-y-6" @submit.prevent="handleCheckout">
        <div class="grid gap-4 md:grid-cols-2">
          <button type="button" class="checkout-choice" :class="{ active: checkoutMode === 'guest' }" @click="checkoutMode = 'guest'">
            ...
            <span class="mt-4 block text-2xl font-black text-white">{{ t('checkout.guestTitle') }}</span>
            <span class="mt-2 block leading-7 text-neutral-400">{{ t('checkout.guestText') }}</span>
          </button>
          <button type="button" class="checkout-choice" :class="{ active: checkoutMode === 'account' }" @click="checkoutMode = 'account'">
            ...
            <span class="mt-4 block text-2xl font-black text-white">{{ t('checkout.accountTitle') }}</span>
            <span class="mt-2 block leading-7 text-neutral-400">{{ t('checkout.accountText') }}</span>
          </button>
        </div>

        <div v-if="checkoutMode === 'account' && !authStore.user" class="premium-panel rounded-2xl p-6 md:p-8">
          <h2 class="text-2xl font-black text-white">{{ t('checkout.signInTitle') }}</h2>
          <p class="mt-3 leading-7 text-neutral-400">{{ t('checkout.signInText') }}</p>
          <div class="mt-6 flex flex-col gap-3 sm:flex-row">
            <NuxtLink to="/auth/login" class="premium-button premium-button-primary">{{ t('nav.login') }}</NuxtLink>
            <NuxtLink to="/auth/register" class="premium-button premium-button-secondary">{{ t('nav.register') }}</NuxtLink>
          </div>
        </div>
        ...
        <div v-else class="premium-panel rounded-2xl p-6 md:p-8">
          <div class="mb-7 flex items-start gap-4">
            ...
            <div>
              <h2 class="text-2xl font-black text-white">{{ t('checkout.shippingInfo') }}</h2>
              <p class="mt-2 text-sm leading-6 text-neutral-400">
                {{ checkoutMode === "guest" ? t('checkout.guestInfo') : t('checkout.profileInfo') }}
              </p>
            </div>
          </div>
        ...
        <div v-if="canSubmit" class="premium-panel rounded-2xl p-6 md:p-8">
          <div class="flex items-center gap-4 rounded-2xl border border-[#FF4D00] bg-[#FF4D00]/10 p-5">
            ...
            <div>
              <p class="font-black text-white">{{ t('checkout.cashOnDelivery') }}</p>
              <p class="mt-1 text-sm text-neutral-400">{{ t('checkout.cashText') }}</p>
            </div>
        ...
        <button v-if="canSubmit" type="submit" :disabled="loading" class="premium-button premium-button-primary w-full text-base active:scale-[0.98] disabled:pointer-events-none disabled:opacity-50">
          ...
          {{ loading ? t('checkout.placing') : checkoutMode === "guest" ? t('checkout.placeGuest') : t('checkout.placeAccount') }}
        </button>
      </form>
      ...
      <aside class="space-y-6 lg:sticky lg:top-28 lg:self-start">
        <div class="premium-panel rounded-2xl p-6 md:p-8">
          ...
          <h2 class="mt-2 text-2xl font-black text-white">{{ t('checkout.orderSummary') }}</h2>
          <div class="mt-6 space-y-5">
            <div v-for="item in cartStore.items" :key="`${item.id}-${item.size}-${item.color}`" class="flex gap-4 border-b border-white/10 pb-5">
              <img :src="item.image" :alt="item.title" width="80" height="80" class="h-20 w-20 rounded-xl object-cover" loading="lazy" decoding="async" />
              ...
              <div class="font-black">{{ formatStorePrice(item.price * item.quantity, locale) }}</div>
            </div>
          </div>
          <div class="mt-6 rounded-2xl border border-white/10 bg-black/40 p-4">
            <label class="block text-sm font-black uppercase tracking-[0.18em] text-neutral-400" for="coupon-code">{{ t('checkout.promoCode') }}</label>
            <div class="mt-3 flex flex-col gap-3 sm:flex-row">
              ...
              <button
                ...
                @click="applyCoupon"
              >
                {{ couponLoading ? t('checkout.applying') : t('checkout.applyCoupon') }}
              </button>
              <button v-else type="button" class="premium-button premium-button-secondary shrink-0" @click="removeCoupon">
                {{ t('checkout.removeCoupon') }}
              </button>
            </div>
            <p v-if="couponMessage" class="mt-3 text-sm font-bold text-emerald-300">{{ couponMessage }}</p>
            <p v-if="couponError" class="mt-3 text-sm font-bold text-red-300">{{ couponError }}</p>
          </div>
          <div class="space-y-4 pt-6">
            <div class="flex justify-between text-neutral-400"><span>{{ t('common.subtotal') }}</span><span class="font-bold text-white">{{ formatStorePrice(cartStore.totalPrice, locale) }}</span></div>
            <div class="flex justify-between text-neutral-400"><span>{{ t('common.shipping') }}</span><span class="font-bold text-emerald-400">{{ t('common.free') }}</span></div>
            <div v-if="appliedCoupon" class="flex justify-between text-neutral-400">
              <span>{{ t('common.discount') }}</span>
              <span class="font-bold text-emerald-400">-{{ formatStorePrice(appliedCoupon.discountAmount, locale) }}</span>
            </div>
            <div class="flex justify-between border-t border-white/10 pt-6 text-2xl font-black"><span>{{ t('common.total') }}</span><span class="text-[#FF4D00]">{{ formatStorePrice(displayTotal, locale) }}</span></div>
          </div>
        </div>
      </aside>
    </div>
...
const displayTotal = computed(() => appliedCoupon.value?.total ?? cartStore.totalPrice);
...
const applyCoupon = async () => {
  ...
  couponMessage.value = t("checkout.applied", { code: appliedCoupon.value.code });
};
