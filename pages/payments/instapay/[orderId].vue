<template>
  <section class="container-premium section-premium">
    <div
      v-if="loading"
      class="premium-panel mx-auto max-w-4xl rounded-2xl p-10 text-center text-neutral-400"
    >
      {{ t("common.loading") }}
    </div>
    ...
    <div
      ...
      class="premium-panel mx-auto max-w-3xl rounded-2xl p-8 text-center"
    >
      <Icon
      ...
      <NuxtLink to="/shop" class="premium-button premium-button-primary mt-7">{{
    ...
    <div v-else-if="order" class="grid gap-8 lg:grid-cols-[1fr_0.8fr]">
      <div class="premium-panel rounded-2xl p-5 md:p-8">
        ...
        <div
          class="mt-7 rounded-2xl border border-[#CF1D1D]/30 bg-[#CF1D1D]/10 p-5"
        >
          <p
            class="text-sm font-black uppercase tracking-[0.18em] text-neutral-400"
          >
          ...
          <div class="mt-2 flex flex-wrap items-center justify-between gap-4">
            ...
            <button
              ...
              @click="copyText(String(order.total_price), 'amount')"
            >
              <Icon name="i-heroicons-clipboard" />
              {{
                copied === "amount"
                  ? t("payments.copied")
                  : t("payments.copyAmount")
              }}
            </button>
          </div>
        </div>
        ...
        <div
          v-if="isPayable"
          class="mt-7 grid gap-5 md:grid-cols-[0.9fr_1.1fr]"
        >
          <div class="rounded-2xl border border-white/10 bg-black/35 p-4">
            ...
            <div
              v-else
              class="flex aspect-square items-center justify-center rounded-xl border border-white/10 bg-black text-center text-sm text-neutral-500"
            >
          ...
          <div class="space-y-4">
            <div class="rounded-2xl border border-white/10 bg-black/35 p-5">
              ...
              <p class="mt-2 text-3xl font-black text-white" aria-live="off">
                {{ timeRemaining }}
              </p>
            </div>
            ...
            <div class="rounded-2xl border border-white/10 bg-black/35 p-5">
              ...
              <p class="mt-2 font-black text-white">
                {{ settings.instapay_account_name || "Viking Store" }}
              </p>
              <p class="mt-4 text-sm text-neutral-500">
                {{ t("payments.instapayId") }}
              </p>
              <div
                class="mt-2 flex flex-wrap items-center justify-between gap-3"
              >
                <p class="font-black text-white">
                  {{ settings.instapay_id || "-" }}
                </p>
                <button
                  v-if="settings.instapay_id"
                  type="button"
                  class="rounded-xl border border-white/10 px-4 py-2 text-sm font-bold transition hover:border-[#CF1D1D]"
                  @click="copyText(settings.instapay_id, 'id')"
                >
                  {{
              ...
              <p class="mt-4 text-sm text-neutral-500">
                {{ t("payments.InstaPayPhoneNumber") }}
              </p>
              <div
                class="mt-2 flex flex-wrap items-center justify-between gap-3"
              >
                <p dir="ltr" class="font-black text-white">
                  {{ instapayPhoneNumber }}
                </p>
                <button
                  type="button"
                  class="rounded-xl border border-white/10 px-4 py-2 text-sm font-bold transition hover:border-[#CF1D1D]"
                  @click="copyText(instapayPhoneNumber, 'phone')"
                >
                  {{
                    copied === "phone" ? t("payments.copied") : t("common.copy")
                  }}
                </button>
              </div>
            </div>
            ...
            <a
              v-if="settings.instapay_payment_link"
              :href="settings.instapay_payment_link"
              target="_blank"
              rel="noopener"
        ...
        <div
          v-if="isPayable"
          class="mt-7 rounded-2xl border border-white/10 bg-black/35 p-5"
        >
          <h2 class="text-2xl font-black text-white">
            {{ t("payments.uploadPaymentProof") }}
          </h2>
          <p class="mt-2 text-sm leading-6 text-neutral-400">
            {{ t("payments.uploadHint") }}
          </p>

          <form class="mt-5 space-y-4" @submit.prevent="submitProof">
            <label class="block">
              <span class="text-sm font-bold text-neutral-400">{{
                t("payments.transactionReference")
              }}</span>
              <input
                v-model="transactionReference"
                class="payment-input mt-2"
                :placeholder="t('payments.transactionReferencePlaceholder')"
              />
            </label>
            <label
              class="block rounded-2xl border border-dashed border-white/15 bg-white/[0.03] p-5 text-center transition hover:border-[#CF1D1D]/70"
            >
              <input
                type="file"
              ...
              <Icon
                ...
                class="mx-auto text-4xl text-[#CF1D1D]"
              />
              <span class="mt-3 block font-bold text-white">{{
                proofFile?.name || t("payments.chooseProof")
              }}</span>
              <span class="mt-1 block text-sm text-neutral-500">{{
                t("payments.proofRules")
              }}</span>
            </label>
            <img
              v-if="proofPreview"
            ...
            <p
              v-if="proofError"
              class="rounded-xl border border-red-500/30 bg-red-500/10 p-3 text-sm font-bold text-red-300"
            >
              {{ proofError }}
            </p>
            <p
              v-if="proofSuccess"
              class="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm font-bold text-emerald-300"
            >
              {{ proofSuccess }}
            </p>
            <button
              ...
              class="premium-button premium-button-primary w-full disabled:pointer-events-none disabled:opacity-50"
            >
              <Icon
                :name="
                  proofSubmitting
                ...
                "
                :class="{ 'animate-spin': proofSubmitting }"
              />
              {{
                proofSubmitting
                  ? t("payments.uploadingProof")
        ...
        <div
          v-else
          class="mt-7 rounded-2xl border border-white/10 bg-black/35 p-6"
        >
          <h2 class="text-2xl font-black text-white">
            {{ statusMessageTitle }}
          </h2>
          ...
          <p
            v-if="order.payment_rejection_reason"
            class="mt-4 rounded-xl border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-200"
          >
            {{ order.payment_rejection_reason }}
          </p>
          <div
            v-if="order.payment_status === 'expired'"
            class="mt-6 flex flex-col gap-3 sm:flex-row"
          >
            <button
              type="button"
              class="premium-button premium-button-primary"
              @click="reorderExpired"
            >
              {{ t("payments.reorder") }}
            </button>
            <NuxtLink
              to="/shop"
              class="premium-button premium-button-secondary"
              >{{ t("payments.continueShopping") }}</NuxtLink
            >
          </div>
        </div>
      </div>
          ...
          <div class="mt-5 space-y-3">
            ...
            >
              ...
              <p class="mt-1 text-sm text-neutral-500">
                {{ item.color || t("shop.default") }} /
                {{ item.size || t("shop.default") }} / {{ t("admin.qty") }}
                {{ item.quantity }}
              </p>
            </article>
          </div>
...
const instapayPhoneNumber = "01123997154";

const accessToken = computed(() => String(route.query.token || ""));
const orderLabel = computed(
  () => order.value?.order_number || String(order.value?.id || "").slice(0, 8),
);
const expiresAt = computed(() =>
  order.value?.payment_expires_at
    ? new Date(order.value.payment_expires_at).getTime()
    : 0,
);
...
const isPayable = computed(
  () =>
    ["awaiting_payment", "rejected"].includes(
      String(order.value?.payment_status || ""),
    ) && secondsLeft.value > 0,
);
...
const paymentTitle = computed(() => {
  if (order.value?.payment_status === "proof_submitted")
    return t("payments.proofSubmittedTitle");
  if (order.value?.payment_status === "paid") return t("payments.paidTitle");
  if (order.value?.payment_status === "expired" || secondsLeft.value <= 0)
    return t("payments.expiredTitle");
  if (order.value?.payment_status === "rejected")
    return t("payments.rejectedTitle");
  return t("payments.completePayment");
});
const statusMessageTitle = computed(() => paymentTitle.value);
const statusMessageText = computed(() => {
  if (order.value?.payment_status === "proof_submitted")
    return t("payments.proofSubmittedText");
  if (order.value?.payment_status === "paid") return t("payments.paidText");
  if (order.value?.payment_status === "rejected")
    return t("payments.rejectedText");
...
const qrUrl = computed(() => {
  ...
  const { data } = supabase.storage
    .from("payment-assets")
    .getPublicUrl(settings.value.instapay_qr_path);
...
const loadPayment = async () => {
  ...
  if (error || data?.ok === false) {
    errorMessage.value =
      error?.message || data?.error || t("payments.invalidAccessText");
    loading.value = false;
    return;
  }
...
const submitProof = async () => {
  ...
  try {
    ...
    const headers = sessionData.session?.access_token
      ...
      : undefined;
    await $fetch("/api/payments/instapay/proof", {
      method: "POST",
      body: form,
      headers,
    });
...
const reorderExpired = async () => {
  const productIds = [
    ...new Set(
      items.value.map((item) => Number(item.product_id)).filter(Boolean),
    ),
  ];
  ...
  const { data } = await supabase
    .from("products")
    .select(
      "id,title,slug,price,cover_image,image,product_variants(id,price,stock_quantity,is_active)",
    )
    .in("id", productIds);
  const products = new Map(
    (data || []).map((product: any) => [Number(product.id), product]),
  );
  const nextItems: any[] = [];
    ...
    const variant = item.variant_id
      ? (product.product_variants || []).find(
          (candidate: any) =>
            ...
            candidate.is_active &&
            Number(candidate.stock_quantity || 0) >= Number(item.quantity || 1),
        )
      : null;
