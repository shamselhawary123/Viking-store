create table if not exists public.product_costs (
  ...
  constraint product_costs_cost_price_nonnegative check (cost_price is null or cost_price >= 0)
);
...
create table if not exists public.product_variant_costs (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint product_variant_costs_cost_price_nonnegative check (cost_price is null or cost_price >= 0)
);

create or replace function public.set_private_product_cost_updated_at()
returns trigger
language plpgsql
...
for each row
execute function public.set_private_product_cost_updated_at();

drop trigger if exists set_product_variant_costs_updated_at on public.product_variant_costs;
create trigger set_product_variant_costs_updated_at
before update on public.product_variant_costs
for each row
execute function public.set_private_product_cost_updated_at();
...
drop policy if exists "admins manage product variant costs" on public.product_variant_costs;
create policy "admins manage product variant costs"
...
revoke all on public.product_variant_costs from anon, authenticated;
...
begin
  ...
  loop
    ...
    v_cost_price := null;
    v_has_cost_price_key := v_variant ? 'cost_price';
    if v_has_cost_price_key then
      v_cost_price := nullif(btrim(coalesce(v_variant->>'cost_price', '')), '')::numeric;
    end if;
    ...
    if v_has_cost_price_key and v_cost_price is not null and v_cost_price < 0 then
      raise exception 'Variant cost price must be zero or more.' using errcode = '23514';
    end if;
  ...
  loop
    ...
    if v_has_cost_price_key then
      if v_cost_price is null then
        delete from public.product_variant_costs
        where variant_id = v_variant_id;
      else
        insert into public.product_variant_costs (variant_id, cost_price)
        values (v_variant_id, v_cost_price)
