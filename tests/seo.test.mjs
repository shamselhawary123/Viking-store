...
const productPageSource = readFileSync(new URL("../pages/shop/[slug].vue", import.meta.url), "utf8");
const sitemapSource = readFileSync(new URL("../server/routes/sitemap.xml.ts", import.meta.url), "utf8");
const robotsSource = readFileSync(new URL("../server/routes/robots.txt.ts", import.meta.url), "utf8");
...
describe("SEO helpers", () => {
  it("builds clean canonical URLs without query strings or duplicate slashes", () => {
    assert.equal(buildCanonicalUrl("https://viking.example/", "/shop?category=boxing"), "https://viking.example/shop");
    assert.equal(buildCanonicalUrl("https://viking.example", "shop/gloves#reviews"), "https://viking.example/shop/gloves");
    assert.equal(buildCanonicalUrl("https://viking.example", "/"), "https://viking.example/");
  });
  ...
  it("keeps product structured data EGP-only and uses real review aggregates only", () => {
    ...
    const withoutReviews = buildProductStructuredData(product, "https://viking.example/shop/elite-boxing-gloves", {
      total: 0,
      average: 0,
    });
    const withReviews = buildProductStructuredData(product, "https://viking.example/shop/elite-boxing-gloves", {
      total: 3,
      average: 4.666,
    });
  ...
  it("keeps private routes out of robots and sitemap output", () => {
    ...
    const sitemap = buildSitemapXml([
      ...
      { loc: "https://viking.example/blog/wrap-guide", lastmod: "2026-08-17T10:00:00.000Z" },
    ]);
  ...
  it("uses the current Vercel production URL as the runtime SEO fallback", () => {
    ...
    assert.equal(buildCanonicalUrl("", "/shop"), "https://viking-store.vercel.app/shop");
    assert.match(buildRobotsTxt(""), /Sitemap: https:\/\/viking-store\.vercel\.app\/sitemap\.xml/);
  });
});
